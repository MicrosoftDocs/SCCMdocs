...
description: Implement and configure the necessary dependencies for deploying applications in Microsoft Endpoint Manager.
ms.date: 11/29/2019
...
*Applies to: Microsoft Endpoint Manager (current branch)*

Use the information in this article to help you implement the necessary dependencies to deploy applications in Microsoft Endpoint Manager.  



## Dependencies external to Microsoft Endpoint Manager
...
To determine automatic user device affinities, the Microsoft Endpoint Manager client reads sign-in events of type **Success** from the Windows security event log. Enable these events with the following two audit policies:
...
## Microsoft Endpoint Manager dependencies
...
To use the reports in Microsoft Endpoint Manager for application management, first install and configure a reporting services point.

For more information, see [Reporting in Microsoft Endpoint Manager](/sccm/core/servers/manage/reporting).  
...
To create virtual applications in Microsoft Endpoint Manager, install App-V 4.6 SP1 or later on devices.
...
These iterative improvements to Software Center and the management point are to simplify your infrastructure and remove the need for the application catalog for user-available deployments. Software Center can deliver all app deployments without the application catalog. Also, if you enable TLS 1.2 and use HTTP with the application catalog, users can't see user-targeted, available deployments. Update Microsoft Endpoint Manager to version 1906 or later to benefit from these improvements.
...
1. Review the default and any custom client settings. In the **Computer Agent** group, make sure the **Default Application Catalog website point** is `(none)` by selecting **Automatically Detect** in the **Select Application Catalog website point** dropdown menu.  
...
> [!NOTE]  
> If you signed in to the device with a Domain Administrator account, the Microsoft Endpoint Manager client doesn't display notification messages. For example, messages indicating that new software is available.  
