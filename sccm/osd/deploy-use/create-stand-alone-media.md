...
- The [Auto Apply Drivers](/sccm/osd/understand/task-sequence-steps#BKMK_AutoApplyDrivers) step in the task sequence. Stand-alone media doesn't support automatic application of device drivers from the driver catalog. Use the [Apply Driver Package](/sccm/osd/understand/task-sequence-steps#BKMK_ApplyDriverPackage) step to make a specified set of drivers available to Windows Setup.  

- The [Download Package Content](/sccm/osd/understand/task-sequence-steps#BKMK_DownloadPackageContent) step in the task sequence. The management point information isn't available on stand-alone media, so the step fails trying to enumerate content locations.  
...
- Dynamic package installs via the [Install Packages](/sccm/osd/understand/task-sequence-steps#BKMK_InstallPackage) step.  

- Dynamic application installs via the [Install Application](/sccm/osd/understand/task-sequence-steps#BKMK_InstallApplication) step.

- The **Use pre-production client package when available** setting in the **Setup Windows and ConfigMgr** task sequence step. For more information about this setting, see [Setup Windows and ConfigMgr](/sccm/osd/understand/task-sequence-steps#BKMK_SetupWindowsandConfigMgr).  

> [!NOTE]  
> An error might occur if your task sequence includes the [Install Package](/sccm/osd/understand/task-sequence-steps#BKMK_InstallPackage) step and you create the stand-alone media at a central administration site. The central administration site doesn't have the necessary client configuration policies. These policies are required to enable the software distribution agent when the task sequence runs. The following error might appear in the **CreateTsMedia.log** file:  
>
...
>
> Alternatively, use a custom [Run Command Line](/sccm/osd/understand/task-sequence-steps#BKMK_RunCommandLine) step. Add it after the [Setup Windows and ConfigMgr](/sccm/osd/understand/task-sequence-steps#BKMK_SetupWindowsandConfigMgr) step and before the first **Install Package** step. The **Run Command Line** step runs the following WMIC command to enable the software distribution agent before the first Install Package step:  
>
